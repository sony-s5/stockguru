...
function parseScreenerHTML(html: string, ticker: string): ScreenerData {
  ...
  const sectionFallbackValue = (section: CheerioAPI, labels: string[]): number | null => {
    const rowMap = buildSectionRowMap($, section)
    const value = findRowValue(rowMap, labels).value
...
'@
$lines[89..181] = $normalize -split "\r?\n"
$lines[547..932] = $parse -split "\r?\n"
