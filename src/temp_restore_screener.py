...
repo_root = Path('src')  # relative from workspace root
file_path = repo_root / 'lib' / 'screener.ts'

# Load original from git HEAD
original_text = subprocess.check_output(
    ['git', 'show', 'HEAD:src/lib/screener.ts'], text=True, encoding='utf-8', errors='ignore'
)
original_lines = original_text.splitlines()

# Replacement blocks as defined in the current parser version
new_normalize = """function normalizeLabel(label: string): string {
  return label
    .replace(/&nbsp;/g, ' ')
    .replace(/\u00A0/g, ' ')
    .replace(/[\u0000-\u001F\u007F]+/g, ' ')
...
new_parse = """function parseScreenerHTML(html: string, ticker: string): ScreenerData {
  const $ = load(html)
...
normalize_start = next(i for i, l in enumerate(original_lines) if l.startswith('function normalizeLabel('))
normalize_end = next(i for i, l in enumerate(original_lines[normalize_start+1:], normalize_start+1) if l.strip() == '}')
parse_start = next(i for i, l in enumerate(original_lines) if l.startswith('function parseScreenerHTML('))
merge_start = next(i for i, l in enumerate(original_lines) if l.strip().startswith('// 4. Merge'))

new_lines = original_lines[:normalize_start] + new_normalize.splitlines() + original_lines[normalize_end+1:parse_start] + new_parse.splitlines() + original_lines[merge_start:]

file_path.write_text('\n'.join(new_lines) + '\n', encoding='utf-8')
print('Rebuilt screener.ts from git HEAD with updated parser')
